...
class PolicyEvaluator:
    ...
    def _cache_key(
        ...
        digest_input = (
            f"{policy.policy_id}:{policy.locale.value}:"
            f"{policy.version}:{business_description}"
        )
    ...
    def _build_system_prompt(policy: ICPTaxonomy) -> str:
        rule_lines = [
            (
                f"- [{rule.rule_id}] risk={rule.risk_level.value} :: "
                f"{rule.description} "
                f"(required_disclosures: "
                f"{'; '.join(rule.required_disclosures) or 'none'}; "
                f"prohibited_terms: "
                f"{'; '.join(rule.prohibited_terms) or 'none'})"
            )
    ...
    async def evaluate(
        ...
        if policy is None:
            raise PolicyNotFoundError(
                f"no policy {policy_id} for locale {locale.value}"
            )
    ...
    async def evaluate_batch(
        self,
        items: list[tuple[str, str, SupportedLocale]],
    ) -> list[EvaluationResult | EvaluationFailure]:
        """Evaluate a batch of items while preserving input order.

        Each item is a tuple of:
            (business_description, policy_id, locale)

        The semaphore bounds concurrent in-flight requests.
        Failures are returned as typed EvaluationFailure entries rather
        than raised, so one bad item does not cancel the rest of the batch.
        """
        tasks = [
            self._evaluate_bounded(i, desc, pid, loc)
            for i, (desc, pid, loc) in enumerate(items)
        ]
